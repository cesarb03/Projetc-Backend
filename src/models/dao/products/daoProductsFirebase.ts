...
class ProductsDAOFirebase extends FirebaseContainer {
  ...
  async getAll(): Promise<any> {
    try {
      const foundItems = await admin.firestore().collection(this.collection).get();
  ...
  async checkProductInCollection(id: any): Promise<any | Error> {
    try {
      const foundItem = await admin.firestore().collection(this.collection).doc(id.toString()).get();
  ...
  async getById(id: any): Promise<any | Error> {
    try {
      ...
      if (foundItem === true) {
        const productSelected = await admin.firestore().collection(this.collection).doc(id.toString()).get();
  ...
  async addProduct(product: any): Promise<any | void> {
    try {
      const productAdded = await admin.firestore().collection(this.collection).add(product);
  ...
  async updateProductById(id: any, newData: any): Promise<any> {
    try {
      const productSelected = await this.checkProductInCollection(id);
      if (productSelected === true) {
        await admin.firestore().collection(this.collection).doc(id.toString()).update(newData);
  ...
  async deleteProductById(id: any): Promise<any> {
    try {
      const productSelected = await this.checkProductInCollection(id);
      if (productSelected === true) {
        await admin.firestore().collection(this.collection).doc(id.toString()).delete();
